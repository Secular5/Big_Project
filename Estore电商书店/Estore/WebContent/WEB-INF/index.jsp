			...
			<ul class="top_bars">
				<c:choose>
					<c:when test="${!empty isLogin}">
						<li>你好 ${isLogin.name}<a href="LoginOut">退出</a>|
						</li>
					</c:when>
					<c:otherwise>
						<li><a href="loginview">亲，请登录!<span class="jt_down"></span></a></li>
					</c:otherwise>
				</c:choose>
						...
						<dd>
							...
							<p>
								<c:forEach items="${firstCategory.categories}"
									var="secondCategory">
									<a href="list.html">${secondCategory.name}</a>
								</c:forEach>
							</p>
						</dd>
					</c:forEach>
				...
				<div class="c3_b1_c_bottom">
					<ul>
						<c:forEach items="${book1}" var="book">
							<li><a href="View?id=${book.id}"><img
									src="${book.image}" width="120px" height="150px" /></a> <a
								href="View">${book.name}</a></li>
						</c:forEach>
					</ul>
				</div>
			</div>
				...
				<c:forEach items="${book2}" var="book">
					<li>
						<div class="c3_b2_txt">
							...
							<p>
								<a href="View?id=${book.id }">更多精彩，点击进入</a>
							</p>
						</div>
					</li>
				</c:forEach>
